import request from '@/utils/request';
import json from './test.json';
const mockServerSettings = require('../../mock/settings');
function getRouter (json, parentId) {
  let layout;
  if (parentId) {
    layout = json.filter((item) => { return item.parent && item.parent.id === parentId });
  } else {
    layout = json.filter((item) => { return !item.parent });
  }
  // console.log(layout)
  const menus = [];
  layout.forEach((item, index) => {
    // const children = json.filter((i) => { return i.parent && item.id === i.parent.id });
    // console.log(children)
    // console.log(item)
    menus.push({
      path: item.menuType === 'directory' ? '/Layout' + index : item.menuValue,
      component: item.menuType === 'directory' ? 'Layout' : '/systemMenu/index.vue',
      redirect: item.menuType === 'directory' ? 'noRedirect' : '',
      name: item.menuName,
      data: item,
      meta: { title: item.menuName, icon: item.icon },
      children: getRouter(json, item.id)
    })
  })
  // console.log(menus)
  return menus
}
export function login () {
  return request({
    url: '/checkFrontSso',
    method: 'get'
  });
}

export function permitThrough () {
  return request({
    url: '/permitThrough',
    method: 'get'
  });
  // return Promise.resolve(false);
}

export function getInfo () {
  // 因为后端暂时未提供登陆接口，暂时返回模式数据
  // return request({
  //   url: '/user',
  //   method: 'get'
  // });
  const data = [
    {
      path: '/example',
      component: 'Layout',
      redirect: 'noRedirect',
      name: 'Example',
      meta: { title: 'example', icon: 'example' },
      children: [
        {
          path: 'table',
          name: 'Table',
          component: '/table/index.vue',
          meta: {
            title: 'table',
            icon: 'table',
            permission: ['update', 'delete', 'create']
          }
        },
        {
          path: 'tree',
          name: 'Tree',
          component: '/tree/index.vue',
          meta: { title: 'tree', icon: 'tree' }
        }
      ]
    },

    {
      path: '/nested',
      component: 'Layout',
      redirect: '/nested/menu1',
      name: 'Nested',
      meta: {
        title: 'nested',
        icon: 'nested'
      },
      children: [
        {
          path: 'menu1',
          component: '/nested/menu1/index.vue', // Parent router-view
          name: 'Menu1',
          meta: { title: 'menu1' },
          children: [
            {
              path: 'menu1-1',
              component: '/nested/menu1/menu1-1/index.vue',
              name: 'Menu1-1',
              meta: { title: 'menu1-1' }
            },
            {
              path: 'menu1-2',
              component: '/nested/menu1/menu1-2/index.vue',
              name: 'Menu1-2',
              meta: { title: 'menu1-2' },
              children: [
                {
                  path: 'menu1-2-1',
                  component: '/nested/menu1/menu1-2/menu1-2-1/index.vue',
                  name: 'Menu1-2-1',
                  meta: { title: 'menu1-2-1' }
                },
                {
                  path: 'menu1-2-2',
                  component: '/nested/menu1/menu1-2/menu1-2-2/index.vue',
                  name: 'Menu1-2-2',
                  meta: { title: 'menu1-2-2' }
                }
              ]
            },
            {
              path: 'menu1-3',
              component: '/nested/menu1/menu1-3/index.vue',
              name: 'Menu1-3',
              meta: { title: 'menu1-3' }
            }
          ]
        },
        {
          path: 'menu2',
          component: '/nested/menu2/index.vue',
          meta: { title: 'menu2' }
        }
      ]
    },
    {
      path: '/form',
      component: 'Layout',
      children: [
        {
          path: 'index',
          name: 'Form',
          component: '/form/index.vue',
          meta: { title: 'form', icon: 'form' }
        }
      ]
    },
    {
      path: 'external-link',
      component: 'Layout',
      children: [
        {
          path: 'http://192.168.0.210:8086/cas',
          meta: { title: 'External_Link', icon: 'link' }
        }
      ]
    }
  ]
  console.log(getRouter(json));
  return Promise.resolve({
    introduction: 'I am a super administrator',
    avatar: 'data:image/png;base64,',
    name: '超级管理员',
    menus: getRouter(json)
  });
}

export function logout () {
  // return request({
  //   url: 'logout',
  //   method: 'get'
  // });
  return Promise.resolve({ casAddr: 'http://' + mockServerSettings.cas.host + ':' + mockServerSettings.cas.port + '/cas/retrieveToken?appId=back-server-sso' });
}
